...
# =========================看你洗的这个狗屎我都服了算了懒得改看你洗的这个狗屎我都服了算了懒得改先用着吧还烦
# 层级损失训练函数
# =========================
...
def train_layerwise_simple():
    ...
    # SwanLab监控
    monitor = SwanlabMonitor(experiment_name="Layerwise_Baseline_CIFAR100")
    monitor.init_experiment(config=config)
...
if __name__ == "__main__":
    ...
    # 如果需要运行多个实验，取消下面的注释
    # results = run_layerwise_experiments()import torch
import torch.nn as nn
import torch.optim as optim
from torchvision import datasets, transforms
from torch.utils.data import DataLoader
